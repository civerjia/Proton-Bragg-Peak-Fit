...
using mex_type = float;
using mex_type_class = mxSINGLE_CLASS; // mxDOUBLE_CLASS mxSINGLE_CLASS
// disable matlab entry function
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
    mex_type *X;
    mex_type *Y;
    mex_type *para;
    X = (mex_type*)mxGetPr(prhs[0]);
    Y = (mex_type*)mxGetPr(prhs[1]);
    para = (mex_type*)mxGetPr(prhs[2]);
    ...
    std::vector<mex_type> X_vec(Nx), Y_vec(Ny);
    std::copy(X, X + Nx, X_vec.begin());
    std::copy(Y, Y + Ny, Y_vec.begin());
    std::vector<mex_type> para_vec(N_para);
    ...
    if((N_para == Nz*N_gaussian*6) | (N_para == Nz*N_gaussian*4))
    {
        if(index == 0)
        {
            ...
            mex_type* dose3d_ptr{};
            dose3d_ptr = (mex_type*)mxGetPr(plhs[0]);
        ...
        else
        {
            ...
            mex_type* grad_ptr{};
            grad_ptr = (mex_type*)mxGetPr(plhs[0]);
            //get gradient

...
void Gauss2d::cuda_interface(std::vector<T> X, std::vector<T> Y, std::vector<T> para, T* dose3D, int Nx, int Ny, int Nz, int N_para, int N_gaussian)
{
    ...
    cudaMemcpy(dose3D, dose3D_dev_ptr, sizeof(T)* dose_size, cudaMemcpyDeviceToHost);
}
template<class T>
void Gauss2d::cuda_interface_gradient(std::vector<T> X, std::vector<T> Y, std::vector<T> para, T* grad, int Nx, int Ny, int Nz, int N_para, int N_gaussian)
{
    ...
    thrust::device_vector<T> Y_dev = Y;
    thrust::device_vector<T> para_dev = para;
	thrust::device_vector<T> grad_dev(grad_size);
    // cast to raw pointer
    T* X_dev_ptr = thrust::raw_pointer_cast(X_dev.data());
    T* Y_dev_ptr = thrust::raw_pointer_cast(Y_dev.data());
    T* para_dev_ptr = thrust::raw_pointer_cast(para_dev.data());
    ...
    // copy data back to host
    cudaMemcpy(grad, grad_dev_ptr, sizeof(T)* grad_size, cudaMemcpyDeviceToHost);
}
